...
from typing import Any, Dict, Generator, Optional, Union, NamedTuple
...
class Temp_RolloutBuffer(BaseBuffer):
    def __init__(
        self,
        buffer_size: int,
        observation_space: spaces.Space,
    ...
    def reset(self) -> None:
        ...
        self.t_1_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
        self.t_2_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
    ...
    def add(
        ...
        self.t_1_observations[self.pos] = np.array(t_1_obs).copy()
        self.t_2_observations[self.pos] = np.array(t_2_obs).copy()
