            ...
            for idx, done in enumerate(dones):
                if done:
                    if infos[idx]['Success'] and not infos[idx]['Collision']:
                            self.success_episode_num.append(1)
                            self.reach_with_collision.append(0)
                    elif infos[idx]['Success'] and infos[idx]['Collision']:
                            self.success_episode_num.append(0)
                            self.reach_with_collision.append(1)
                    else:
                        self.success_episode_num.append(0)
                        self.reach_with_collision.append(0)
                    if (
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if log_interval is not None and self.iteration % log_interval == 0:
                ...
                if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
                    ...
                    self.logger.record("rollout/success_rate", np.mean(self.success_episode_num))
                    self.logger.record("rollout/reach_with_collision", np.mean(self.reach_with_collision))
                # For debug
