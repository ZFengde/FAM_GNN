...
class Temp_OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.fail_episode_num = deque(maxlen=100)
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            for idx, done in enumerate(dones):
                if done:
                    if infos[idx]['Success']:
                            self.success_episode_num.append(1)
                            self.fail_episode_num.append(0)
                    elif infos[idx]['Collision']:
                            self.success_episode_num.append(0)
                            self.fail_episode_num.append(1)
                    else:
                        self.success_episode_num.append(0)
                        self.fail_episode_num.append(0)
                    if (
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if log_interval is not None and self.iteration % log_interval == 0:
                ...
                if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
                    ...
                    self.logger.record("rollout/collision_rate", np.mean(self.fail_episode_num))
                # For debug
